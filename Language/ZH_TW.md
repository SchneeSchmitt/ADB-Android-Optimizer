...
- `Qualcomm Only`：**僅提供**給使用高通SoC/CPU晶片裝置的優化。
- `Compile`：建議在您暫時不需要使用裝置的時候運行，這個腳本需要運行大約30分鐘。建議在運行過程中，在開發者選項中啟用「充電時保持螢幕開啟」功能。過程中您的設備溫度也會升高。
- `Extra Boost`：提供更強的性能，但是耗電量也會大幅提升。
- `Power Saving`：提升手機續航，減低性能和耗電量，並且調節系統的運行模式。
- `Lite Power Saving`：平衡手機性能和耗電量，適合日常使用。
...
12. 如果你想要裝置進一步提升性能或者省電，你可以運行`Extra_Boost`提升性能**或者**`Power_saving`進一步省電。否則，你可以運行`Lite Power Saving`來維持日常所需的性能和續航。
